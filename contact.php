...
<body>
	...
	<div class="banner-page-content">
		<div class="banner-subcontent">
			<form action="" method="post" class="banner-contact-form">
				<input type="email" name="email" value="<?php if(!empty($_SESSION['login'])) echo $user->email; ?>" placeholder="Enter your email">
                <input type="text" name="name" value="<?php if(!empty($_SESSION['login'])) echo $user->name; ?>" placeholder="Enter your name">
	...
	if(isset($_POST['email']) && isset($_POST['text'])){
	        ...
	        $data = array(     "to" => array("kilroyevent@gmail.com"=>""),
	                        "from" => array("$email","$name"),
	                        "subject" => "Contact",
	                        "text" => $text
	            );
	            $mailin->send_email($data);
	            echo "<span class='valid'>Successfully sent!!!</span>";
	}
